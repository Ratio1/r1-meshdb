...
type UDFPrivate struct {
	...
	// MultiColDataSource is true if the function may return multiple columns. This
	// is only the case if the UDF returns a RECORD type and is used as a data
	// source.
	...
	for in.cache.Next() {
		if existing, ok := in.cache.Item().(*UDFExpr); ok {
			if in.hasher.IsScalarListExprEqual(val.Args, existing.Args) &&
				...
				in.hasher.IsTypeEqual(val.Typ, existing.Typ) &&
				in.hasher.IsBoolEqual(val.SetReturning, existing.SetReturning) &&
				...
				in.hasher.IsBoolEqual(val.MultiColDataSource, existing.MultiColDataSource) {
				return existing
			}
		}
	}
