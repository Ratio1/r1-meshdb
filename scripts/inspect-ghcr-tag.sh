...
tagged_ref="${1:?usage: inspect-ghcr-tag.sh <ghcr-version-or-stable-tag>}"
: "${GHCR_USERNAME:?GHCR_USERNAME is required}"
: "${GHCR_TOKEN:?GHCR_TOKEN is required}"

if [[ ! "${tagged_ref}" =~ ^ghcr\.io/ratio1/r1db:(v(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)|stable)$ ]]; then
  echo "invalid R1DB GHCR tag: ${tagged_ref}" >&2
  exit 2
fi
tag="${BASH_REMATCH[1]}"
...
token_response="$({
  printf 'url = "https://ghcr.io/token?service=ghcr.io&scope=repository%%3Aratio1%%2Fr1db%%3Apull"\n'
...
headers="${tmp}/headers"
status="$({
  printf 'url = "https://ghcr.io/v2/ratio1/r1db/manifests/%s"\n' "${tag}"
  printf 'head\n'
