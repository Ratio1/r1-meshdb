...
func GetKeywordID(k string) int32 {
	...
	case "deallocate":
		return DEALLOCATE
	case "debug_dump_metadata_sst":
		return DEBUG_DUMP_METADATA_SST
	case "debug_ids":
		return DEBUG_IDS
	case "debug_pause_on":
	...
	case "encrypted":
		return ENCRYPTED
	case "encryption_info_dir":
		return ENCRYPTION_INFO_DIR
	...
	case "force_index":
		return FORCE_INDEX
	case "force_inverted_index":
		return FORCE_INVERTED_INDEX
	case "force_not_null":
	...
	case "include_all_virtual_clusters":
		return INCLUDE_ALL_VIRTUAL_CLUSTERS
	case "including":
		return INCLUDING
	case "increment":
		return INCREMENT
	...
	case "skip_missing_udfs":
		return SKIP_MISSING_UDFS
	...
	case "unlogged":
		return UNLOGGED
	case "unsafe_restore_incompatible_version":
		return UNSAFE_RESTORE_INCOMPATIBLE_VERSION
	case "unset":
		return UNSET
	case "unsplit":
		return UNSPLIT
	...
	case "update":
		return UPDATE
	case "updates_cluster_monitoring_metrics":
		return UPDATES_CLUSTER_MONITORING_METRICS
	case "upsert":
		return UPSERT
	case "use":
		return USE
	...
	case "virtual_cluster_name":
		return VIRTUAL_CLUSTER_NAME
	case "visible":
		return VISIBLE
	case "volatile":
		return VOLATILE
