...
prefix="r1db-ci-${run_id}"
[[ "${#prefix}" -le 40 ]]
tmp="$(mktemp -d /tmp/r1db-real-cloudflare.XXXXXXXX)"
allocation_dir="${tmp}/cloudflare"
certs_dir="${tmp}/certs"
nodes=("r1db-real-1-${run_id}" "r1db-real-2-${run_id}" "r1db-real-3-${run_id}")
networks=("r1db-real-net-1-${run_id}" "r1db-real-net-2-${run_id}" "r1db-real-net-3-${run_id}")
stores=("r1db-real-store-1-${run_id}" "r1db-real-store-2-${run_id}" "r1db-real-store-3-${run_id}")
db_password="r1_real_cloudflare_validation_password"
cleanup_started=false
evidence_dir="${R1DB_EVIDENCE_DIR:-${R1_SQL_EVIDENCE_DIR:-}}"
...
preserve_cleanup_state() {
  ...
  if [[ -n "${evidence_dir}" ]]; then
    ...
    destination="$(mktemp /tmp/r1db-cloudflare-cleanup-state.XXXXXXXX.json)"
  fi
