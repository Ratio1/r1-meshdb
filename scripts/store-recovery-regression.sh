...
assert_log "${hostile_store_mode_case}" "invalid R1DB store directory"
...
assert_log "${hostile_store_owner_case}" "invalid R1DB store directory"
...
if [[ "$(inspect_store "${legacy_store_root}" "stat -c '%a' /store")" != "700" ]]; then
  echo "legacy R1DB store directory was not migrated to mode 700" >&2
  exit 1
fi
...
fixture_check="$(docker run --rm -v "${real_fixture_volume}:/store:ro" \
  ...
  debug pebble sstable check "${fixture_sst}" 2>&1)"
if ! grep -Fq 'checksum mismatch' <<< "${fixture_check}"; then
  echo "R1DB checksum verification did not detect the injected SSTable corruption" >&2
  exit 1
fi
...
if [[ -e "${capture_file}" ]]; then
  echo "R1DB started after recovery logs disappeared" >&2
  exit 1
fi
...
if [[ -e "${capture_file}" ]]; then
  echo "R1DB started after the exact recorded recovery run disappeared" >&2
  exit 1
fi
...
if [[ -e "${topology_capture}" ]]; then
  echo "R1DB started after recovery topology changed" >&2
  exit 1
fi
...
if [[ -e "${count_capture}" ]]; then
  echo "R1DB started after recovery node count changed" >&2
  exit 1
fi
...
if [[ -e "${ca_capture}" ]]; then
  echo "R1DB started after recovery CA changed" >&2
  exit 1
fi
...
if [[ -e "${capture_file}" ]]; then
  echo "R1DB started after recovery was exhausted" >&2
  exit 1
fi
...
if [[ -e "${capture_file}" ]]; then
  echo "R1DB started after a recovery log read failure" >&2
  exit 1
fi
...
assert_log "${prune_find_failure_case}" "could not enumerate R1DB run-log entries"
if [[ -e "${capture_file}" ]]; then
  echo "R1DB started after run-log pruning enumeration failed" >&2
  exit 1
fi
...
assert_log "${log_root_mode_case}" "invalid R1DB log directory"
...
if [[ "$(inspect_store "${log_root_legacy_store}" "stat -c '%a' /store/logs")" != "700" ]]; then
  echo "legacy R1DB log directory was not migrated to mode 700" >&2
  exit 1
fi
...
assert_log "${log_root_owner_case}" "invalid R1DB log directory"
...
assert_log "${run_mode_case}" "invalid R1DB run-log entry"
...
assert_log "${run_owner_case}" "invalid R1DB run-log entry"
...
assert_log "${mounted_run_case}" "invalid R1DB run-log entry"
...
assert_log "${rm_failure_case}" "could not remove old R1DB run-log entry"
...
assert_log "${logs_symlink_case}" "invalid R1DB log directory"
if find "${external_logs}" -mindepth 1 -print -quit | grep -q .; then
  echo "symlinked log directory escaped the R1DB store" >&2
  exit 1
fi
