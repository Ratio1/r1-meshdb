...
require_digest="${R1DB_REQUIRE_DIGEST:-${R1_SQL_REQUIRE_DIGEST:-true}}"
...
run_id="r1db-rolling-$$-${RANDOM}"
network="${run_id}"
legacy_overlay="r1db-legacy-transport:${run_id}"
candidate_overlay="r1db-candidate-transport:${run_id}"
nodes=("${run_id}-1" "${run_id}-2" "${run_id}-3")
tmp="$(mktemp -d /tmp/r1db-rolling.XXXXXXXX)"
certs_dir="${tmp}/certs"
