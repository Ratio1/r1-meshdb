...
if [[ "${R1DB_SECRETS_REEXEC:-}" != "1" ]]; then
  ...
  secret_dir="$(mktemp -d /tmp/r1db-secrets.XXXXXXXX)"
  ...
  exec env \
    ...
    R1DB_SECRETS_REEXEC=1 \
    R1DB_SECRET_DIR="${secret_dir}" \
...
: "${R1DB_SECRET_DIR:?internal secret directory is required}"
case "${R1DB_SECRET_DIR}" in
  /tmp/r1db-secrets.*) ;;
  *) echo "[deeploy-crdb] invalid internal secret directory" >&2; exit 1 ;;
esac
if [[ -L "${R1DB_SECRET_DIR}" || ! -d "${R1DB_SECRET_DIR}" ||
      "$(stat -c '%u:%a' "${R1DB_SECRET_DIR}" 2>/dev/null || true)" != "0:700" ]]; then
  echo "[deeploy-crdb] invalid internal secret directory" >&2
  exit 1
fi
R1DB_PASSWORD_FILE="${R1DB_SECRET_DIR}/database-password"
R1DB_TUNNEL_TOKEN_FILE="${R1DB_SECRET_DIR}/cloudflare-token"
R1DB_STAGED_CERT_FILES=(
  "${R1DB_SECRET_DIR}/ca-crt"
  "${R1DB_SECRET_DIR}/node-crt"
  "${R1DB_SECRET_DIR}/node-key"
  "${R1DB_SECRET_DIR}/client-root-crt"
  "${R1DB_SECRET_DIR}/client-root-key"
)
for secret_file in "${R1DB_PASSWORD_FILE}" "${R1DB_TUNNEL_TOKEN_FILE}" \
    "${R1DB_STAGED_CERT_FILES[@]}"; do
...
early_secret_cleanup() {
  rm -f -- "${R1DB_PASSWORD_FILE}" "${R1DB_TUNNEL_TOKEN_FILE}" \
    "${R1DB_STAGED_CERT_FILES[@]}" >/dev/null 2>&1 || true
  rmdir "${R1DB_SECRET_DIR}" >/dev/null 2>&1 || true
}
trap early_secret_cleanup EXIT
CRDB_PASSWORD="$(cat "${R1DB_PASSWORD_FILE}")"
export -n CRDB_PASSWORD R1DB_SECRET_DIR R1DB_PASSWORD_FILE R1DB_TUNNEL_TOKEN_FILE
...
prepare_store_root() {
  ...
  if [[ -L "${CRDB_STORE}" || ! -d "${CRDB_STORE}" || \
    ...
    log "invalid R1DB store directory"
    return 1
  fi
...
case "${CRDB_USER,,}" in
  root|admin|node|public)
    log "CRDB_USER must not be a reserved R1DB identity"
    exit 1
    ;;
esac
...
prune_old_run_logs() {
  ...
  if ! find "${CRDB_ACTIVE_STORE}/logs" -mindepth 1 -maxdepth 1 \
    ...
    log "could not enumerate R1DB run-log entries"
    return 1
  fi
  if ! rm -f -- "${run_log_list_file}"; then
    log "could not remove R1DB run-log enumeration state"
    return 1
  fi
  run_log_list_file=""
  for record in "${run_dirs[@]}"; do
    ...
    if [[ "${entry_type}" != "d" || \
          ...
          -L "${run_dir}" ]]; then
      log "invalid R1DB run-log entry"
      return 1
    fi
    ...
    if ! rm -rf -- "${run_dir}" || [[ -e "${run_dir}" || -L "${run_dir}" ]]; then
      log "could not remove old R1DB run-log entry"
      return 1
    fi
  done
}
...
prepare_current_run_log_dir() {
  ...
  if [[ -L "${log_root}" ]]; then
    log "invalid R1DB log directory"
    return 1
  fi
  if [[ ! -e "${log_root}" ]]; then
  ...
  if [[ -L "${CRDB_CURRENT_RUN_LOG_DIR}" || \
    ...
    log "could not establish a private current-run R1DB log directory"
    return 1
  fi
}
...
cleanup() {
  ...
  remove_sensitive_temp_file "${R1DB_PASSWORD_FILE}"
  ...
  remove_sensitive_temp_file "${R1DB_TUNNEL_TOKEN_FILE}"
  rmdir "${R1DB_SECRET_DIR}" >/dev/null 2>&1 || true
  if [[ "${recovery_handler_was_active}" == "true" ]]; then
...
cloudflared tunnel --no-autoupdate run \
  --token-file "${R1DB_TUNNEL_TOKEN_FILE}" \
...
log "starting R1DB node ${CRDB_NODE_ID} with join ${join_list}"
...
/cockroach/cockroach start \
  "${start_flags[@]}" &
crdb_pid="$!"
register_required_process "${crdb_pid}" "R1DB"
...
if ! run_guarded_operation "SQL listener readiness" "${sql_readiness_pid}"; then
  log "R1DB SQL listener did not open on ${CRDB_BIND_HOST}:${CRDB_SQL_PORT} within ${CRDB_BOOTSTRAP_TIMEOUT_SECONDS} seconds"
  exit 1
fi
...
if [[ "${CRDB_NODE_ID}" == "1" ]]; then
  ...
  if [[ "${CRDB_RECOVERY_ACTIVE}" == "true" ]]; then
    ...
    log "initializing R1DB cluster if needed"
    ...
    if [[ "${init_status}" != "0" ]]; then
      if grep -qiE "already initialized|cluster has already been initialized" <<< "${init_output}"; then
        log "R1DB cluster is already initialized"
      else
        printf '%s\n' "${init_output}" >&2
        exit "${init_status}"
      fi
  ...
  log "waiting for R1DB SQL bootstrap readiness"
...
SQL
    ...
    log "ensuring R1DB database and configured administrator exist"
    cat >> "${bootstrap_sql}" <<SQL
