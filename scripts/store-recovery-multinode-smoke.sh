...
if [[ "$(docker inspect -f '{{.State.ExitCode}}' "${nodes[0]}")" != "137" ]]; then
  echo "recovery node did not preserve the killed R1DB status" >&2
  exit 1
fi
...
if [[ -z "${new_node_id}" || "${new_node_id}" == "${old_node_id}" ]]; then
  echo "recovered node did not join with a new R1DB node ID" >&2
  exit 1
fi
...
if [[ "$(docker inspect -f '{{.State.ExitCode}}' "${nodes[0]}")" != "1" ]]; then
  echo "exhausted recovery did not fail closed before starting R1DB" >&2
  exit 1
fi
