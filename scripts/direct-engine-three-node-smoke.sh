...
mkdir -p "${tmp}/certs" "${tmp}/roach1" "${tmp}/roach2" "${tmp}/roach3"
...
for _ in $(seq 1 120); do
  docker exec "${nodes[0]}" /cockroach/cockroach init \
    --certs-dir=/certs --host=roach1:26257 >/dev/null 2>&1 || true
  if docker exec "${nodes[0]}" /cockroach/cockroach sql \
...
docker exec "${nodes[0]}" /cockroach/cockroach sql \
...
[[ "${replication_ready}" == "true" ]] || { echo "direct cluster remained under-replicated" >&2; exit 1; }

count="$(docker exec "${nodes[2]}" /cockroach/cockroach sql \
...
for _ in $(seq 1 180); do
  restart_count="$(docker exec "${nodes[2]}" /cockroach/cockroach sql \
    --certs-dir=/certs --host=roach3:26257 --format=csv \
