...
class EphemeralTunnelTests(unittest.TestCase):
  def test_allocate_persists_tokens_only_in_owner_readable_files(self):
    ...
    with tempfile.TemporaryDirectory() as tmp:
      ...
      state = allocate(client(fake), output, 3, "r1db-ci", lambda: next(suffixes))
  ...
  def test_partial_failure_deletes_created_dns_then_tunnels(self):
    ...
    with tempfile.TemporaryDirectory() as tmp:
      with self.assertRaises(CloudflareError):
        allocate(client(fake), Path(tmp) / "allocation", 3, "r1db-ci", lambda: "fixed")
  ...
  def test_lost_tunnel_create_response_is_discovered_and_cleaned_by_name(self):
    ...
    with tempfile.TemporaryDirectory() as tmp:
      output = Path(tmp) / "allocation"
      with self.assertRaises(CloudflareError):
        allocate(client(fake), output, 1, "r1db-ci", lambda: "lost")
  ...
  def test_partial_failure_keeps_recoverable_state_when_cleanup_fails(self):
    ...
    with tempfile.TemporaryDirectory() as tmp:
      ...
      with self.assertRaisesRegex(CloudflareError, "cleanup left 2.*HTTP 409"):
        allocate(client(fake), output, 3, "r1db-ci", lambda: "fixed")
  ...
  def test_load_state_binds_recovery_to_the_expected_run_prefix(self):
    with tempfile.TemporaryDirectory() as tmp:
      state_path = Path(tmp) / "state.json"
      name = "r1db-ci-12345-2-1-deadbeef"
      state_path.write_text(json.dumps({
        "schemaVersion": 1,
        "accountId": "account",
        "zoneId": "zone",
  ...
  def test_cleanup_run_prefix_deletes_only_the_exact_attempt_namespace(self):
    ...
    self.assertEqual(cleanup_run_prefix(client(fake), "r1db-ci-12345-2", retries=1), 1)
    ...
    self.assertEqual(cleanup_run_prefix(client(fake), "r1db-ci-12345-2", retries=1), 0)
  ...
  def test_cleanup_run_prefix_rejects_ambiguous_or_oversized_matches(self):
    ...
    with self.assertRaisesRegex(CloudflareError, "run prefix"):
      cleanup_run_prefix(client(fake), "r1db-ci-12345", retries=1)
    self.assertFalse(fake.calls)

    fake.tunnels = {
      f"tunnel-{index}": {
        "id": f"tunnel-{index}",
        "name": f"r1db-ci-12345-2-{index}-deadbeef",
        "config_src": "local",
      }
      for index in range(1, 5)
    }
    with self.assertRaisesRegex(CloudflareError, "more than three"):
      cleanup_run_prefix(client(fake), "r1db-ci-12345-2", retries=1)
  ...
  def test_recovery_state_ids_never_drive_deletion(self):
    ...
    self.assertEqual(
      cleanup_state_allocations(
        client(fake),
        state,
        expected_run_prefix="r1db-ci-12345-2",
        retries=1,
      ),
      1,
    )
